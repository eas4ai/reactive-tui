...
use reactive_tui::core::surface::{Rgba, Attr};
...
fn snapshot_list_widget() {
    ...
    let bg = Rgba { r: 0.0, g: 0.0, b: 0.2, a: 1.0 };
    let selected_bg = Rgba { r: 0.0, g: 0.2, b: 0.4, a: 1.0 };
    ...
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::MoveTo { x: 0, y: 0 })));
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::SetAttributes(Attr::BOLD))));
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::PrintRun(s) if s.contains("Item 2"))));
}

#[test]
fn snapshot_paragraph_widget() {
    let mut builder = RenderOpsBuilder::new();
    let fg = Rgba { r: 0.9, g: 0.9, b: 0.9, a: 1.0 };
    let bg = Rgba { r: 0.1, g: 0.1, b: 0.1, a: 1.0 };
...
fn snapshot_table_widget() {
    let mut builder = RenderOpsBuilder::new();
    let header_fg = Rgba { r: 1.0, g: 1.0, b: 0.0, a: 1.0 };
    let header_bg = Rgba { r: 0.2, g: 0.2, b: 0.0, a: 1.0 };
    let cell_fg = Rgba { r: 1.0, g: 1.0, b: 1.0, a: 1.0 };
    let cell_bg = Rgba { r: 0.0, g: 0.0, b: 0.0, a: 1.0 };
    ...
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::SetAttributes(Attr::BOLD))));
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::PrintRun(s) if s.contains("Name"))));
}

#[test]
fn snapshot_styled_text() {
    let fg_normal = Rgba { r: 0.8, g: 0.8, b: 0.8, a: 1.0 };
    let fg_keyword = Rgba { r: 0.2, g: 0.6, b: 1.0, a: 1.0 };
    let bg = Rgba { r: 0.0, g: 0.0, b: 0.0, a: 1.0 };
    
    // Simulate syntax highlighted code
    let line = StyledLineBuilder::new()
        .fg(fg_keyword).bg(bg).attr(Attr::BOLD).text("fn")
        .fg(fg_normal).bg(bg).attr(Attr::empty()).text(" main() {")
        .build();
    ...
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::MoveTo { x: 4, y: 10 })));
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::SetAttributes(Attr::BOLD))));
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::PrintRun(s) if s == "fn")));
}

#[test]
fn snapshot_clear_operations() {
    let mut builder = RenderOpsBuilder::new();
    let bg = Rgba { r: 0.1, g: 0.1, b: 0.3, a: 1.0 };
    ...
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::ClearScreen)));
    assert!(ops.ops().iter().any(|op| matches!(op, 
        RenderOp::ClearArea { x: 10, y: 5, width: 20, height: 10, .. })));
}
...
fn snapshot_cursor_operations() {
    ...
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::SetCursorVisible(false))));
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::SaveCursorPosition)));
    assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::RestoreCursorPosition)));
}
...
fn snapshot_synchronized_output() {
    ...
    assert!(matches!(ops.ops().first(), Some(RenderOp::SetSynchronizedOutput(true))));
    assert!(matches!(ops.ops().last(), Some(RenderOp::SetSynchronizedOutput(false))));
}
...
fn test_animate_none_parsing() {
    ...
    let test_cases = vec![
        ...
        ("bg-blue-500 animate-pulse text-white", vec!["pulse".to_string()]),
    ];
    
    for (class_str, expected) in test_cases {
        let result = extract_css_animations_from_classes(class_str);
        println!("Class: '{}' -> Animations: {:?} (expected: {:?})", class_str, result, expected);
...
fn test_animate_none_integration() {
    ...
    let element = Element::component_with_props("DebugComponent", DebugProps {
...
fn test_animate_none_vs_regular() {
    ...
    let regular_element = Element::component_with_props("CompareComponent", DebugProps {
    ...
    // Test 2: animate-none
    let none_element = Element::component_with_props("CompareComponent", DebugProps {
        text: "None".to_string(),
    })

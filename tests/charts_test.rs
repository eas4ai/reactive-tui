use reactive_tui::component::{Element, registry::{register_component, global_cleanup_all}};
use reactive_tui::widgets::display::{
    Chart, ChartProps, ChartType, ChartAxis, ChartLegend,
...
fn test_chart_types() {
    ...
    // Test line chart
    let line_chart = ChartProps::line_chart(data.clone())
        .with_title("Line Chart Example");
    
    // Test pie chart
    let pie_chart = ChartProps::pie_chart(data.clone())
        .with_title("Pie Chart Example");
...
fn test_chart_configuration() {
    ...
    assert_eq!(chart_props.title, Some("Advanced Chart Configuration".to_string()));

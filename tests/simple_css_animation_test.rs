...
fn test_css_animation_system_works() {
    ...
    assert!(available.len() >= 4, "Should have at least 4 built-in animations");
    ...
    let element = Element::component_with_props("TestComponent", TestProps {
    ...
    assert!(stats.total_animations > 0, "Should have at least 1 active animation");
    assert!(stats.active_components > 0, "Should have at least 1 animated component");
...
fn test_css_animation_parsing_works() {
    ...
    let element = Element::component_with_props("ParseTestComponent", TestProps {
    ...
    assert!(stats.total_animations >= 1, "Should have parsed at least 1 animation from class string");
...
fn test_css_animation_cleanup_works() {
    ...
    {
        let element = Element::component_with_props("CleanupTestComponent", TestProps {
        ...
        assert!(during_stats.total_animations > 0, "Should have animations while component exists");
...
fn test_animate_none_works() {
    ...
    for (class_str, expected_count) in test_cases {
        ...
        println!("Class '{}' -> {} animations (expected: {})", class_str, animations.len(), expected_count);
        assert_eq!(animations.len(), expected_count,
                   "animate-none parsing failed for: '{}'", class_str);
    }
...
fn test_css_animation_integration_end_to_end() {
    ...
    for (animation_class, text) in components {
        let element = Element::component_with_props("E2ETestComponent", TestProps {
            text: text.to_string(),
        })
    ...
    println!("Added {} animations across {} components", animations_added, components_added);
    
    assert!(animations_added > 0, "Should have added animations");
    assert!(components_added > 0, "Should have added animated components");

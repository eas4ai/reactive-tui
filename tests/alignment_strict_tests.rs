...
fn self_align_in_column_exact_positions() {
    ...
    use std::borrow::Cow;
    let root = NodeSpec { class: Cow::from("w-10 h-5 flex flex-col items-start"), text: None, children: vec![
        NodeSpec { class: Cow::from("self-start w-1"), text: Some(Cow::from("A")), children: vec![] },
        NodeSpec { class: Cow::from("self-center w-1"), text: Some(Cow::from("B")), children: vec![] },
        NodeSpec { class: Cow::from("self-end w-1"), text: Some(Cow::from("C")), children: vec![] },
    ]};
    ...
    assert_eq!(a, 0);
    assert!(b == 4 || b == 5, "center x should be around middle, got {b}");
    assert_eq!(c, 9);
}
...
fn place_items_center_in_row_exact_y() {
    ...
    use std::borrow::Cow;
    let root = NodeSpec { class: Cow::from("w-20 h-5 flex flex-row place-items-center"), text: None, children: vec![
        NodeSpec { class: Cow::from("w-1 h-1"), text: Some(Cow::from("X")), children: vec![] },
    ]};

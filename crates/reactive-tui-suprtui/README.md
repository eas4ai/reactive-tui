...
Unicode segmentation, image composition and links. It is a separate package
so that `reactive-tui` can resolve entirely from crates.io once both are
published; until then its manifest sets `publish = false`.

        ...
        try:
            ...
            if host == "kitty":
                args = ["kitty", "--config", "NONE", "--override", "linux_display_server=x11",
                        ...
                        "--listen-on", "unix:" + str(temp / "kitty.sock"), "--override", "allow_remote_control=socket-only", "--dump-commands", "--dump-bytes", str((output / "terminal.bin").resolve()), *program]
            elif host == "ghostty":
            ...
            for stage in range(3):
                ...
                if stage == 0:
                    state = subprocess.run(["kitty", "+kitten", "@", "--to", "unix:" + str(temp / "kitty.sock"), "kitten", "/tmp/abi-004-kitty-state.py"], env=env, capture_output=True, timeout=10)
                    (output / "image-state.json").write_bytes(state.stdout)
                    (output / "image-state.err").write_bytes(state.stderr)

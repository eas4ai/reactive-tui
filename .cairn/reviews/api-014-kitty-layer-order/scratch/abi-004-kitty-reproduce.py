...
for index in range(3):
 directory=Path('/tmp/abi-004-kitty-diagnostic-'+str(index));directory.mkdir(exist_ok=False)
 with (directory/'driver.out').open('w') as out:
  result=subprocess.run(['/usr/bin/python3','-B','/tmp/abi-004-kitty-debug-capture.py','kitty',str(directory),'kitty'],env=env,stdout=out,stderr=subprocess.STDOUT,timeout=90)
...
Path('/tmp/abi-004-kitty-diagnostics.json').write_text(json.dumps(results,indent=2)+'\n')

...
static void send_file(const char *name) {
    ...
    snprintf(path, sizeof(path), "/tmp/abi-004-kitty-reference-frames/%s.bin", name);
...
int main(int argc, char **argv) {
    ...
    raw = saved;
    /* Control: preserve the original fixture canonical terminal mode. */
    ...
    while (now() - start < 45) {
        ...
        if (stage != shown) {
            ...
            if (stage < 2) {
                char header[8], pixels[8];
                snprintf(header, sizeof(header), "%d-head", stage);
                snprintf(pixels, sizeof(pixels), "%d-image", stage);
                send_file(header);
                struct timespec gap = {.tv_sec = 1, .tv_nsec = 0};

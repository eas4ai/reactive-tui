...
def run(host, output, protocol=None):
    ...
    with tempfile.TemporaryDirectory(prefix="rtui-image-host-") as temp:
        ...
        program = ["/tmp/abi-004-kitty-reference", str(command), str(output)]
        ...
        try:
            ...
            if host == "kitty":
                args = ["kitty", "--config", "NONE", "--override", "linux_display_server=x11",
                        ...
                        "--dump-commands", "--dump-bytes", str((output / "terminal.bin").resolve()), *program]
            elif host == "ghostty":
            ...
            for stage in range(3):
                ...
                if stage == 1:
                    time.sleep(1)
                    control = output / "stage-1-after-wait.png"
                    subprocess.run(["/usr/bin/import", "-window", "root", str(control)], env=env, check=True, timeout=10)
                    counts = Image.open(control).convert("RGB").getcolors(1000 * 700)
                    measured = {name: sum(count for count, actual in counts if actual == color) for name, color in colors.items()}
                    (output / "after-wait.json").write_text(json.dumps(measured) + "\n")
                    subprocess.run(["xrefresh"], env=env, check=True, timeout=10)
                    time.sleep(0.2)
                    extra = output / "stage-1-after-expose.png"
                    subprocess.run(["/usr/bin/import", "-window", "root", str(extra)], env=env, check=True, timeout=10)
                    counts = Image.open(extra).convert("RGB").getcolors(1000 * 700)
                    measured = {name: sum(count for count, actual in counts if actual == color) for name, color in colors.items()}
                    (output / "after-expose.json").write_text(json.dumps(measured) + "\n")

...
impl DirectGridLayout {
    ...
    fn compute_layout(&mut self) {
        ...
        let col_width = if self.grid.cols > 0 {
            (self.total_width.saturating_sub(self.grid.gap * (self.grid.cols - 1))) / self.grid.cols
        } else {
            self.total_width
        };
        
        let row_height = if self.grid.rows > 0 {
            (self.total_height.saturating_sub(self.grid.gap * (self.grid.rows - 1))) / self.grid.rows
        } else {
            self.total_height
        };
    ...
    fn compute_area_placement(&self, area: &GridArea, col_width: usize, row_height: usize) -> GridPlacement {
        // Calculate position
    ...
    fn parse_background_color(&self, area: &GridArea) -> crate::core::surface::Rgba {
        if let Some(ref css_class) = area.css_class {
            if css_class.contains("bg-red-500") {
                return crate::core::surface::Rgba::new(0.937, 0.267, 0.267, 1.0); // red-500
            } else if css_class.contains("bg-green-500") {
                return crate::core::surface::Rgba::new(0.133, 0.773, 0.369, 1.0); // green-500
            } else if css_class.contains("bg-blue-500") {
                return crate::core::surface::Rgba::new(0.231, 0.510, 0.965, 1.0); // blue-500
            } else if css_class.contains("bg-yellow-500") {
                return crate::core::surface::Rgba::new(0.918, 0.702, 0.031, 1.0); // yellow-500
            } else if css_class.contains("bg-purple-500") {
                return crate::core::surface::Rgba::new(0.659, 0.333, 0.969, 1.0); // purple-500
            } else if css_class.contains("bg-pink-500") {
                return crate::core::surface::Rgba::new(0.925, 0.282, 0.600, 1.0); // pink-500
            } else if css_class.contains("bg-gray-400") {
                return crate::core::surface::Rgba::new(0.612, 0.639, 0.686, 1.0); // gray-400

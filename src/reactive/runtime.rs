...
impl ReactiveRuntime {
    ...
    fn flush_pending_effects(&self) {
        let pending = self.pending_effects.borrow_mut().drain().collect::<Vec<_>>();
...
impl RuntimeContext {
    ...
    pub fn create_effect(&self, f: impl FnOnce() -> Option<Box<dyn FnOnce()>> + 'static) -> EffectId {
...
thread_local! {
    ...
    static RUNTIME: RefCell<Option<RuntimeContext>> = RefCell::new(None);
}

...
impl StyledRun {
    ...
    pub fn plain(text: impl Into<String>) -> Self {
        Self {
            ...
            bg: Rgba { r: 0.0, g: 0.0, b: 0.0, a: 1.0 },
            attr: Attr::empty(),
        }
    }
...
impl StyledLineBuilder {
    ...
    pub fn new() -> Self {
        Self {
            line: StyledLine::new(),
            current_fg: Rgba { r: 1.0, g: 1.0, b: 1.0, a: 1.0 },
            current_bg: Rgba { r: 0.0, g: 0.0, b: 0.0, a: 1.0 },
...
mod tests {
    ...
    fn test_styled_line_merging() {
        ...
        assert_eq!(line.runs.len(), 1, "Adjacent runs with same style should merge");
    ...
    #[test]
    fn test_builder() {
        let fg_red = Rgba { r: 1.0, g: 0.0, b: 0.0, a: 1.0 };
        let fg_green = Rgba { r: 0.0, g: 1.0, b: 0.0, a: 1.0 };
        let bg = Rgba { r: 0.0, g: 0.0, b: 0.0, a: 1.0 };
    ...
    fn test_to_render_ops() {
        ...
        assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::MoveTo { x: 10, y: 5 })));
        assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::SetFgColor(_))));
        assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::SetBgColor(_))));
        assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::SetAttributes(_))));
        assert!(ops.ops().iter().any(|op| matches!(op, RenderOp::PrintRun(s) if s == "Test")));
    }
}
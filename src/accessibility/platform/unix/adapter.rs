...
use crate::accessibility::platform::translation::{
    next_adapter_id, ActionHandlerNoMut, ActionHandlerWrapper, Adapter as AdapterImpl,
    AdapterCallback, AppContext, CacheEvent, Event, FullNodeId, PlatformNode, WindowBounds,
};
...
use crate::accessibility::platform::unix::{
    context::new_app_context,
    transport::{MessageSender, Result, Transport},
};
...
impl Callback {
    pub(super) fn new(messages: MessageSender) -> Self {
        Self { messages }
    }
...
impl Adapter {
    ...
    pub fn new(
        ...
        let context = new_app_context();
        let transport = Transport::new(context.clone(), wake)?;
    ...
    pub(crate) fn send_message(&self, message: Message) {
        self.transport.sender.send(message);
    }
        ...
        match &mut *state {
            ...
            AdapterState::Pending {
                ...
                let r#impl = AdapterImpl::with_wrapped_action_handler(
                    self.id,
                    &self.context,
                    Callback::new(self.transport.sender.clone()),
                    initial_state,
                    *is_window_focused,
                    *root_window_bounds,
...
impl Drop for Adapter {
    fn drop(&mut self) {
        // Transport Drop cancels and joins before the retained state is released.
        if let Err(error) = self.close() {
            log::debug!("screen-reader adapter closed after failure: {error}");
        }
    }
}

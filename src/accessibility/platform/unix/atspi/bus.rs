...
use crate::accessibility::platform::translation::{
    AppContext, DocumentEvent, FullNodeId, NodeIdOrRoot, ObjectEvent, PlatformNode, PlatformRoot,
...
impl Bus {
    pub(crate) async fn new(
        ...
        bus.register_root_node(context).await?;
        Ok(bus)
    }
    ...
    async fn register_root_node(&mut self, context: &Arc<RwLock<AppContext>>) -> Result<()> {
        let node = PlatformRoot::new(context);

...
use accesskit::{
    Action, ActionData, ActionRequest, Affine, AriaCurrent, Live, NodeId, Orientation, Point, Rect,
    Role, Toggled, TreeId,
};
...
impl NodeWrapper<'_> {
    ...
    fn filtered_child_ids(&self) -> impl DoubleEndedIterator<Item = FullNodeId> + '_ {
    ...
    pub(crate) fn role(&self) -> AtspiRole {
        ...
        match self.0.role() {
            ...
            Role::Pane => AtspiRole::Panel,
            Role::ScrollView => AtspiRole::ScrollPane,
    ...
    pub(crate) fn state(&self, is_window_focused: bool) -> StateSet {
        ...
        if state
            .aria_current()
            .is_some_and(|current| current != AriaCurrent::False)
        {
    ...
    pub(super) fn attributes(&self) -> HashMap<&'static str, String> {
        ...
        if let Some(current) = self.0.aria_current() {
            ...
            attributes.insert("current", value.into());
        }
    ...
    fn supports_component(&self) -> bool {
        self.0.raw_bounds().is_some()
            || self.is_root()
            || self.0.data().supports_action(Action::Focus)
    }

...
use crate::core::surface::{Surface, Rgba, Attr, Cell};
...
impl TextEditor {
    ...
    pub fn render(&self, surface: &mut Surface, x: usize, y: usize) {
        let text_fg = Rgba { r: 0.9, g: 0.9, b: 0.9, a: 1.0 };
        let text_bg = Rgba { r: 0.1, g: 0.1, b: 0.1, a: 1.0 };
        let line_num_fg = Rgba { r: 0.5, g: 0.5, b: 0.5, a: 1.0 };
        let selection_bg = Rgba { r: 0.2, g: 0.4, b: 0.6, a: 1.0 };
        let cursor_bg = Rgba { r: 0.8, g: 0.8, b: 0.8, a: 1.0 };
        ...
        for line_idx in self.visible_lines() {
            ...
            if self.show_line_numbers {
                ...
                surface.write_str(x + self.line_number_width, current_y, " ", line_num_fg, text_bg, Attr::empty());
            }
            ...
            for (char_idx, ch) in line_text.chars().enumerate() {
                ...
                if pos == cursor_pos {
                    bg = cursor_bg;
                    fg = Rgba { r: 0.0, g: 0.0, b: 0.0, a: 1.0 };
                }
            ...
            if cursor_pos == line_end && line_idx == self.buffer.pos_to_line_col(cursor_pos).0 {
                if current_x < x + self.width {
                    surface.set(current_x, current_y, Cell {
                        ch: ' ',
                        fg: Rgba { r: 0.0, g: 0.0, b: 0.0, a: 1.0 },
                        bg: cursor_bg,
                        attr: Attr::empty(),
    ...
    pub fn get_styled_lines(&self) -> Vec<StyledLine> {
        ...
        for line_idx in self.visible_lines() {
            ...
            if self.show_line_numbers {
                let line_num = format!("{:>width$} ", line_idx + 1, width = self.line_number_width);
                line.push(StyledRun::new(line_num, line_num_fg, text_bg, Attr::empty()));
            }
            ...
            if let Some((sel_start, sel_end)) = selection_range {
                if line_end > sel_start && line_start < sel_end {
                    ...
                    // Add remaining text
                    if !current_text.is_empty() {
                        line.push(StyledRun::new(current_text, text_fg, text_bg, Attr::empty()));
                    }
                } else {
...
mod tests {
    ...
    fn test_editor_selection() {
        ...
        assert_eq!(editor.cursor.selected_text(&editor.buffer), Some("World".to_string()));

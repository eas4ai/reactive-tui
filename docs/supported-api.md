...
passes become stale when an input changes. API-018 documents the public surface,
API-019 verifies the residual runtime contracts, and API-020 reconciles the full
audit and capture-process lifetime.
...
| `hooks` | State/effects/timers, clipboard, gestures and shared/scoped references have retained runtime coverage | [API-004](../.cairn/mechanisms/api-hook-lifecycle.md), [API-008](../.cairn/mechanisms/api-clipboard.md), [API-019](../.cairn/mechanisms/api-residual.md) | Gesture state is component-owned; local references require the approved App-supplied scope and remain on their creator thread; [residual inventory](residual-api-inventory.md) | Covered by named checks |
...
| `ui` | Public UI helpers and App-owned Updater dispatch with ordered, retained updates | [API-019](../.cairn/mechanisms/api-residual.md); [residual inventory](residual-api-inventory.md) | An Updater belongs to its App and does not route updates through global state | Covered by named checks |
...
| `animation`, `screen` | Typed keyframe interpolation, current-value relative animation, retained screen input and visible transition midpoints | [API-013](../.cairn/mechanisms/api-animation-screens.md), [API-010](../.cairn/mechanisms/api-paint-properties.md), [API-019](../.cairn/mechanisms/api-residual.md) | [Animation guide](ANIMATION_INTEGRATION.md) explains caller-driven ScreenManager updates and approved cell approximations; legacy ID, custom-property and hardware-preference fields are compatibility metadata ignored by terminal painting | Covered by named checks |
...
| `render`, `vdom` | Render trees, reconciliation, retained legacy patch route and menu node constructors | [API-016](../.cairn/mechanisms/api-entry-points.md), [API-011](../.cairn/mechanisms/api-widget-behavior.md), [API-019](../.cairn/mechanisms/api-residual.md) | Wide/deep conversion and destruction use bounded explicit work stacks; nested legacy events retain capture/target/bubble order; a patch list is not a terminal frame | Covered by named checks |
| `editor` | Unicode scalar position model, grapheme-aligned selection, insertion/deletion/movement and native editor access | [API-007](../.cairn/mechanisms/api-editor-unicode.md), [API-017](../.cairn/mechanisms/api-native-components.md), [API-019](../.cairn/mechanisms/api-residual.md) | Public editors expose no undo/redo contract; do not interpret a byte offset as a scalar position | Covered by named checks |
| `markdown`, `syntax` | Public comrak-backed conversion, StyledLine rendering, optional Syntect highlighting and checked 1 MiB input bounds | [API-018](../.cairn/mechanisms/api-documentation.md), [API-019](../.cairn/mechanisms/api-residual.md) | Checked entry points reject oversized input before parsing or highlighting | Covered by named checks |
| `platform`, `terminal`, `escape` | Retained platform adapters, child PTYs, bounded streaming parsing and owned terminal control; native clipboard/terminal records | [API-008](../.cairn/mechanisms/api-clipboard.md), [API-011](../.cairn/mechanisms/api-widget-behavior.md), [API-016](../.cairn/mechanisms/api-entry-points.md), [API-019](../.cairn/mechanisms/api-residual.md) | [Entry points](application-entry-points.md); [owned Unix input migration](unix-input.md); Windows ConPTY remains selected; raw mode and Unix workers are released only by their final library owner | Covered by named checks |
| `display`, `theme` | App-owned performance metrics and adaptive FPS; inherited, mutable and isolated theme/color helpers | [API-015](../.cairn/mechanisms/api-features.md), [API-019](../.cairn/mechanisms/api-residual.md) | [Performance guide](ADAPTIVE_PERFORMANCE.md); App-owned performance handles replace global App routing; Theme inheritance is explicit and no automatic App theme-provider API is claimed | Covered by named checks |
...
API-020 reconciles every audit row with current behavior evidence and verifies that
normal and forced-timeout image captures reap their private host and Xvfb groups.

...
| Unix input worker | One live descriptor owner and bounded queued input; shutdown reaps the worker before descriptor reuse | A worker reads a reused descriptor, outlives shutdown or queues without a bound | Pending API-019 |
...
| Full claimed native platform surface | Every retained macOS/Windows/Unix path is tied to native evidence or an explicitly approved scope choice | A platform claim relies only on Linux compilation or an untested placeholder | Pending API-019 |
